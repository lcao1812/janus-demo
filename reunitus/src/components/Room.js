...
class Room extends React.Component {
	...
	onPublisherMessage(msg, jsep) {
		...
		if (event) {
			if (event === 'joined') {
				...
				if (publishers) {
					console.log(
						`RECEIVED a list of ${publishers.length} active publishers in the room:`
					);
					publishers.forEach((publisher) => {
						...
						console.log(
							`Id: ${id} username: ${displayName} audiocodec: ${audio} videocodec: ${video}`
						);
	...
	newRemoteFeed(id, display, audiocodec, videocodec) {
		...
		janus.attach({
			...
			success: (subHandle) => {
				...
				console.log(
					`Attached to ${remoteFeed.getPlugin()} | feedid=${id}`
				);
				console.log('--- NEW SUBSCRIBE HANDLE ---');
			...
			onmessage: (msg, jsep) => {
				...
				if (jsep) {
					...
					remoteFeed.createAnswer({
						...
						error: (err) => {
							console.error(
								'Could not establish subscriber connection:',
								err
							);
						}
					});
			...
			onremotestream: (stream) => {
				...
				if (!videoTrack || videoTrack.length == 0)
					console.error('No video tracks for sub:', id);
				if (!audioTrack || audioTrack.length == 0)
					console.error('No audio tracks for sub:', id);

				// We have received the stream requested, catalog it and do UI housekeeping
				if (
					this.state.remoteStreamObjs.filter((o) => o.id == id)
						.length != 0
				)
					return;
			...
			oncleanup: () => {
				...
				console.log(
					`Subscription connection closed! id: ${id}, display: ${display}`
				);
				const newList = this.state.remoteStreamObjs.filter(
					(o) => o.id != id
				);
	...
	decreaseVideoIndex = async () => {
		await this.setState({
			startVideoIndex: this.state.startVideoIndex - 2
		});
		console.log('start index', this.state.startVideoIndex);
	};

	increaseVideoIndex = async () => {
		await this.setState({
			startVideoIndex: this.state.startVideoIndex + 2
		});
		console.log('start index', this.state.startVideoIndex);
	...
	renderVideoFeeds = () => {
		if (
			this.state.startVideoIndex + 2 <
			this.state.remoteStreamObjs.length
		) {
			{
				let sub = this.state.remoteStreamObjs.slice(
					this.state.startVideoIndex,
					this.state.startVideoIndex + 2
				);
				console.log(sub);
				return sub.map((v) => <RemoteFeed key={v.id} {...v} />);
			}
		} else {
			{
				return this.state.remoteStreamObjs
					.slice(
						this.state.startVideoIndex,
						this.state.remoteStreamObjs.length
					)
					.map((v) => <RemoteFeed key={v.id} {...v} />);
			}
		}
	};
	...
	render() {
		...
		let upPane = (
			<div>
				<Button
					onClick={this.decreaseVideoIndex}
					disabled={this.state.startVideoIndex <= 0}
					className="Arrow-control-button mb-3"
				>
					UP
				</Button>
			</div>
		...
		let downPane = (
			<div>
				<Button
					onClick={this.increaseVideoIndex}
					disabled={
						this.state.startVideoIndex >=
						this.state.remoteStreamObjs.length - 2
					}
					className="Arrow-control-button mb-3"
				>
					DOWN
		...
		let videoPane = <div>{this.renderVideoFeeds()}</div>;

		return (
			<div className="App">
				<header className="App-header">
					<h1>
						Room: {this.props.roomid} | {this.props.username}
					</h1>
					<div>
				...
				<Row className="justify-content-md-center">{upPane}</Row>

				<Row>
					{
						// render only the selected videos
						videoPane
					}
				</Row>

				<Row className="justify-content-md-center">{downPane}</Row>

				<Row>
					{
						// render the audio for all connected feeds
						this.state.remoteStreamObjs.map((v) => (
							<RemoteAudio key={v.id + '_a'} {...v} />
						))
					}
				</Row>
			</div>
